...
Contabilidad de Odoo](https://www.odoo.com/page/accounting-features), y haga
...
Odoo utiliza múltiples servicios web para conectar con los bancos:
  ...
  * [Salt Edge](bank_synchronization/saltedge.html): Global

  * [Ponto](bank_synchronization/ponto.html): Europa

  * [Enable Banking](bank_synchronization/enablebanking.html): Países escandinavos
...
de Odoo Enterprise. También usamos un proxy entre su base de datos y el
...
Si tiene algún inconveniente durante la primera sincronización, verifique que
su navegador web no bloquee las ventanas emergentes y que el bloqueador de
...
contables en su base de datos de Contabilidad de Odoo. Por ejemplo, si importa
su saldo de cierre en Odoo el 31/12/2022 y comienza a registrar transacciones
contables el 01/01/2023, su fecha de sincronización debería ser el 01/01/2023.
...
automáticamente. En el caso de estas instituciones usted recibe un mensaje de
error en el que se le pide que desactive la sincronización automática durante
la sincronización automática de la cuenta. Este mensaje se puede encontrar en
el chatter de sus sincronizaciones en línea. En este caso, asegúrese de
...
[Odoo](https://www.odoo.com/help), vaya a Contabilidad‣ Configuración ‣
...
Si no puede volver a conectarse con el botón Obtener cuenta póngase en
contacto con [soporte](https://www.odoo.com/help) directamente con su ID de
cliente o la referencia del error que aparece en el chatter.

## Proceso de migración para usuarios que hayan instalado Odoo antes de
diciembre de 2020

Si usa Odoo local, primero asegúrese de que su fuente esté actualizada con la
última versión de Odoo.
...
Nota
  ...
  * Todas las sincronizaciones anteriores se desconectan durante la instalación y ya no vuelven a funcionar.
...
control de contabilidad y haga clic en el botón Sincronizar ahora. También
puede sincronizar y obtener transacciones a través del Panel de control de
contabilidad ‣ Configuración ‣ Contabilidad: Sincronización en línea. También
puede sincronizar y obtener transacciones a través del Panel de control de
contabilidad ‣ Configuración ‣ Contabilidad: Sincronización en línea. Algunos
proveedores solo permiten una actualización por día, por lo que es posible que
al hacer clic en Sincronizar ahora no se muestren sus transacciones más
recientes si ya ha realizado esa acción previamente en el mismo día.
...
Odoo no ayuda con problemas técnicos que ocurren con los bancos en la fase
