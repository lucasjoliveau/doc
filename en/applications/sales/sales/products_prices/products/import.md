...
Odoo _Sales_ provides a template for importing products with categories and
...
adjusted and customized, and then uploaded back into the Odoo database.
...
in Odoo Sales.](../../../../../_images/gear-import-records-option.png)
...
![The Import records option selectable from the gear icon on the Products page
in Odoo Sales.](../../../../../_images/import-template-products.png)
...
Internal Reference column for each product can be helpful in many cases. This
can even be from previous software spreadsheets to ease the transition into
Odoo.
...
To manually match a column name with a field in Odoo, click the Odoo Field
...
After customizing the product template spreadsheet, return to the Odoo product
...
configured product template spreadsheet, separated by File Column, Odoo Field,
and Comments.
...
If everything is lined up and applied correctly, Odoo reveals a blue banner at
...
Once those errors are fixed, click Test again to ensure all necessary issues
have been remedied appropriately.

If additional product template spreadsheets need to be uploaded, click the
Load File button, select the desired product template spreadsheet, and repeat
the process.

When everything is ready, click the Import button.

When clicked, Odoo instantly imports those products, and reveals the main
Products page, with a pop-up message in the upper-right corner. This pop-up
message informs the user how many products were successfully imported.

![The pop-up window that appears after a successful product import process in
Odoo Sales.](../../../../../_images/successful-import-popup.png)
...
**must** be imported _first_ from their own list menu.
...
been uploaded, click Import, and Odoo returns to the Products page.
...
follows: Attribute, Display Type, Variant Creation Mode, and Values / Value.
...
The Variants Creation Mode **cannot** be changed once the attribute is used on
...
spreadsheet, it’s time to import and upload it into Odoo. To do that, navigate
...
uploaded, click Import, and Odoo returns to the Attributes page. That’s where
those newly-added attributes and values can be found and edited, if necessary.
...
To import multiple values, separate them by _just_ a comma, **not** a comma
...
the spreadsheet, it’s time to import and upload them into Odoo. To do that,
...
has been uploaded, click Import, and Odoo returns to the Products page. That’s
where the newly-added products can be found.

To view and modify the attributes and variants on any products, select the
desired product from the Products page, and click the Attributes & Variants
tab.

See also

[Product variants](variants.html)
